...
|----------|------|----------|
| UE5 source (canonical) | `<UE5_SRC>/Engine/Source/` | Reference implementation for `DataBunch`, `NetConnection`, `PackageMapClient`, `RepLayout`, `ActorChannel`. **This is the official source we cross-check against** — do NOT use `C:/Program Files/Epic Games/UE_*` (those are stripped engine installs). |
...
UE5 source at `<UE5_SRC>/Engine/Source/Runtime/Engine/Private/`
has:
